import type { Bank } from '@packages/general-codes/src/types'

const banks = [
  {
    name: 'Finbricks MOCK_COBS',
    shortName: 'Finbricks MOCK_COBS',
    code: '5051',
    country: 'Czechia',
    swiftBic: 'FBRKMOCKXXX',
  },
  {
    name: 'Komerční banka, a.s.',
    shortName: 'KB',
    code: '0100',
    country: 'Czechia',
    swiftBic: 'KOMBCZPPXXX',
  },
  {
    name: 'Československá obchodní banka, a. s.',
    shortName: 'ČSOB',
    code: '0300',
    country: 'Czechia',
    swiftBic: 'CEKOCZPP',
  },
  {
    name: 'MONETA Money Bank, a.s.',
    shortName: 'MONETA',
    code: '0600',
    country: 'Czechia',
    swiftBic: 'AGBACZPP',
  },
  {
    name: 'ČESKÁ NÁRODNÍ BANKA',
    shortName: 'ČNB',
    code: '0710',
    country: 'Czechia',
    swiftBic: 'CNBACZPP',
  },
  {
    name: 'Česká spořitelna, a.s.',
    shortName: 'ČS',
    code: '0800',
    country: 'Czechia',
    swiftBic: 'GIBACZPX',
  },
  {
    name: 'Fio banka, a.s.',
    shortName: 'Fio',
    code: '2010',
    country: 'Czechia',
    swiftBic: 'FIOBCZPP',
  },
  {
    name: 'Bank of Tokyo-Mitsubishi UFJ (Holland)',
    shortName: 'MUFG',
    code: '2020',
    country: 'Czechia',
    swiftBic: 'BOABORPP',
  },
  {
    name: 'AKCENTA, spořitelní a úvěrní družstvo',
    shortName: 'AKCENTA',
    code: '2030',
    country: 'Czechia',
    swiftBic: 'AKCZCZPP',
  },
  {
    name: 'Citfin, spořitelní družstvo',
    shortName: 'Citfin',
    code: '2060',
    country: 'Czechia',
    swiftBic: 'CITFCZPP',
  },
  {
    name: 'TRINITY BANK a.s.',
    shortName: 'Trinity',
    code: '2070',
    country: 'Czechia',
    swiftBic: 'TRINCZPP',
  },
  {
    name: 'ČSOB Hypoteční banka, a.s.',
    shortName: 'ČSOB Hypo',
    code: '2100',
    country: 'Czechia',
    swiftBic: 'CEKOCZPP',
  },
  {
    name: 'Peněžní dům, spořitelní družstvo',
    shortName: 'Peněžní dům',
    code: '2200',
    country: 'Czechia',
    swiftBic: 'PENDCZPP',
  },
  {
    name: 'ERB bank, a.s.',
    shortName: 'ERB',
    code: '2210',
    country: 'Czechia',
    swiftBic: 'ABORCZPP',
  },
  {
    name: 'Artesa, spořitelní družstvo',
    shortName: 'Artesa',
    code: '2220',
    country: 'Czechia',
    swiftBic: 'ARTESCZPP',
  },
  {
    name: 'Poštová banka, a.s.',
    shortName: 'Poštová banka',
    code: '2240',
    country: 'Czechia',
    swiftBic: 'POBNCZPP',
  },
  {
    name: 'Banka CREDITAS a.s.',
    shortName: 'Creditas',
    code: '2250',
    country: 'Czechia',
    swiftBic: 'CREDITAS',
  },
  {
    name: 'NEY spořitelní družstvo',
    shortName: 'NEY',
    code: '2260',
    country: 'Czechia',
    swiftBic: 'NEYSCZPP',
  },
  {
    name: 'ZUNO BANK AG',
    shortName: 'ZUNO',
    code: '2310',
    country: 'Czechia',
    swiftBic: 'ZUNOCZPP',
  },
  {
    name: 'Citibank Europe plc',
    shortName: 'Citibank',
    code: '2600',
    country: 'Czechia',
    swiftBic: 'CITIUS33',
  },
  {
    name: 'UniCredit Bank Czech Republic and Slovakia, a.s.',
    shortName: 'UniCredit',
    code: '2700',
    country: 'Czechia',
    swiftBic: 'BACXCZPP',
  },
  {
    name: 'Air Bank a.s.',
    shortName: 'Air Bank',
    code: '3030',
    country: 'Czechia',
    swiftBic: 'AIRACZPP',
  },
  {
    name: 'Českomoravská stavební spořitelna, a.s.',
    shortName: 'ČMSS',
    code: '3050',
    country: 'Czechia',
    swiftBic: 'CABORCZP',
  },
  {
    name: 'PKO BP S.A.',
    shortName: 'PKO BP',
    code: '3060',
    country: 'Czechia',
    swiftBic: 'BPKOPLCX',
  },
  {
    name: 'ING Bank N.V.',
    shortName: 'ING',
    code: '3500',
    country: 'Czechia',
    swiftBic: 'INGBCZPP',
  },
  {
    name: 'Expobank CZ a.s.',
    shortName: 'Expobank',
    code: '4000',
    country: 'Czechia',
    swiftBic: 'EXPNCZPP',
  },
  {
    name: 'Národní rozvojová banka, a.s.',
    shortName: 'NRB',
    code: '4300',
    country: 'Czechia',
    swiftBic: 'NRBACZPP',
  },
  {
    name: 'Raiffeisenbank a.s.',
    shortName: 'Raiffeisen',
    code: '5500',
    country: 'Czechia',
    swiftBic: 'RZBCCZPP',
  },
  {
    name: 'The Royal Bank of Scotland plc',
    shortName: 'RBS',
    code: '5400',
    country: 'Czechia',
    swiftBic: 'NAIACZPP',
  },
  {
    name: 'J&T BANKA, a.s.',
    shortName: 'J&T',
    code: '5800',
    country: 'Czechia',
    swiftBic: 'JTBACZPP',
  },
  {
    name: 'PPF banka a.s.',
    shortName: 'PPF',
    code: '6000',
    country: 'Czechia',
    swiftBic: 'PPFBCZPP',
  },
  {
    name: 'Equa bank a.s.',
    shortName: 'Equa',
    code: '6100',
    country: 'Czechia',
    swiftBic: 'EQBKCZPP',
  },
  {
    name: 'COMMERZBANK Aktiengesellschaft',
    shortName: 'Commerzbank',
    code: '6200',
    country: 'Czechia',
    swiftBic: 'COBACZPP',
  },
  {
    name: 'mBank S.A., organizační složka',
    shortName: 'mBank',
    code: '6210',
    country: 'Czechia',
    swiftBic: 'BREXCZPP',
  },
  {
    name: 'BNP Paribas S.A.',
    shortName: 'BNP Paribas',
    code: '6300',
    country: 'Czechia',
    swiftBic: 'BNPACZPP',
  },
  {
    name: 'Partners Banka, a.s.',
    shortName: 'Partners',
    code: '6363',
    country: 'Czechia',
    swiftBic: 'PARTCZPP',
  },
  {
    name: 'Všeobecná úverová banka a.s.',
    shortName: 'VÚB',
    code: '6700',
    country: 'Czechia',
    swiftBic: 'VUBACZPP',
  },
  {
    name: 'Sberbank CZ, a.s. v likvidaci',
    shortName: 'Sberbank',
    code: '6800',
    country: 'Czechia',
    swiftBic: 'SBERCZPP',
  },
  {
    name: 'Waldviertler Sparkasse Bank AG',
    shortName: 'Waldviertler',
    code: '7940',
    country: 'Czechia',
    swiftBic: 'SPWTCZPP',
  },
  {
    name: 'Deutsche Bank Aktiengesellschaft Filiale',
    shortName: 'Deutsche Bank',
    code: '7910',
    country: 'Czechia',
    swiftBic: 'DEUTCZPX',
  },
  {
    name: 'Raiffeisen stavební spořitelna a.s.',
    shortName: 'Raiffeisen SS',
    code: '7950',
    country: 'Czechia',
    swiftBic: 'RZSBKZPP',
  },
  {
    name: 'ČSOB Stavební spořitelna, a.s.',
    shortName: 'ČSOB SS',
    code: '7960',
    country: 'Czechia',
    swiftBic: 'CEKOCZPP',
  },
  {
    name: 'Wüstenrot hypoteční banka a.s.',
    shortName: 'Wüstenrot',
    code: '7980',
    country: 'Czechia',
    swiftBic: 'WUSTCZPP',
  },
  {
    name: 'MONETA Stavební Spořitelna, a.s.',
    shortName: 'MONETA SS',
    code: '7970',
    country: 'Czechia',
    swiftBic: 'AGBACZPP',
  },
  {
    name: 'Modrá pyramida stavební spořitelna, a.s.',
    shortName: 'Modrá pyramida',
    code: '7990',
    country: 'Czechia',
    swiftBic: 'MPSSCZPP',
  },
  {
    name: 'Volksbank Raiffeisenbank Nordoberpfalz eG',
    shortName: 'Volksbank',
    code: '8030',
    country: 'Czechia',
    swiftBic: 'VBANDE77',
  },
  {
    name: 'Oberbank AG pobočka Česká republika',
    shortName: 'Oberbank',
    code: '8040',
    country: 'Czechia',
    swiftBic: 'OBKLCZ2X',
  },
  {
    name: 'Stavební spořitelna České spořitelny, a.s.',
    shortName: 'SSČS',
    code: '8060',
    country: 'Czechia',
    swiftBic: 'GIBACZPX',
  },
  {
    name: 'Česká exportní banka, a.s.',
    shortName: 'ČEB',
    code: '8090',
    country: 'Czechia',
    swiftBic: 'CEBACZPP',
  },
  {
    name: 'HSBC Continental Europe, Czech Republic',
    shortName: 'HSBC',
    code: '8150',
    country: 'Czechia',
    swiftBic: 'HSBCCZPP',
  },
  {
    name: 'Sparkasse Oberlausitz-Niederschlesien',
    shortName: 'Sparkasse',
    code: '8190',
    country: 'Czechia',
    swiftBic: 'WELADED1GRL',
  },
  {
    name: 'FAS finance company s.r.o.',
    shortName: 'FAS',
    code: '8198',
    country: 'Czechia',
    swiftBic: 'FASFCZPP',
  },
  {
    name: 'PRIVAT BANK AG der Raiffeisenlandesbank Oberösterreich',
    shortName: 'PRIVAT BANK',
    code: '8200',
    country: 'Czechia',
    swiftBic: 'PRVACZPP',
  },
  {
    name: 'Payment execution s.r.o.',
    shortName: 'Payment execution',
    code: '8220',
    country: 'Czechia',
    swiftBic: 'PAYECZPP',
  },
  {
    name: 'EEPAYS s.r.o.',
    shortName: 'EEPAYS',
    code: '8230',
    country: 'Czechia',
    swiftBic: 'EEPACZPP',
  },
  {
    name: 'Družstevní záložna Kredit',
    shortName: 'DZ Kredit',
    code: '8240',
    country: 'Czechia',
    swiftBic: 'DZKREDIT',
  },
  {
    name: 'Bank of China (CEE) Ltd.',
    shortName: 'Bank of China',
    code: '8250',
    country: 'Czechia',
    swiftBic: 'BKCHCZPP',
  },
  {
    name: 'Bank of Communications Co., Ltd.',
    shortName: 'Bank of Communications',
    code: '8255',
    country: 'Czechia',
    swiftBic: 'COMMCZPP',
  },
  {
    name: 'Industrial and Commercial Bank of China Limited',
    shortName: 'ICBC',
    code: '8265',
    country: 'Czechia',
    swiftBic: 'ICBKCZPP',
  },
  {
    name: 'Multitude Bank p.l.c.',
    shortName: 'Multitude',
    code: '8500',
    country: 'Czechia',
    swiftBic: 'MULTCZPP',
  },
  {
    name: 'Test - Devizová burza a.s.',
    shortName: 'Test - Devizová burza a.s.',
    code: '8610',
    country: 'Czechia',
    swiftBic: '',
  },
] as const satisfies readonly Bank[]

export default banks

export type BankCode = (typeof banks)[number]['code']
